  ...
  async invokeLocalNodeJs(handlerPath, handlerName, event, customContext) {
    ...
    function handleError(err) {
      ...
      if(!this.options.finalOutput || this.options.finalOutput === 'console') {
        ...
        writeText(`ERROR, details in file: ${_outputPath}.`);
        fse.writeFileSync(_outputPath, JSON.stringify(errorResult, null, 4));
      }
      process.exitCode = 1;
    }
    ...
    function handleResult(result) {
      ...
      if(!this.options.finalOutput || this.options.finalOutput === 'console') {
        ...
        let _outputPath = path.join(path.dirname(pathToHandler), '_functionOutput.log.json');
        writeText(`SUCCESS, details in file: ${_outputPath}.`);
        fse.writeFileSync(_outputPath, JSON.stringify(result, null, 4));
      }
    }
